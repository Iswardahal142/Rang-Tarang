...
// ── OAuth: Refresh token se access token lo ──────────────
...
export async function GET() {
  ...
  try {
    ...
    // ── Playlist items — OAuth se private/scheduled bhi aayenge ──
    const playlistRes = await fetch(
      `https://www.googleapis.com/youtube/v3/playlistItems?part=snippet,contentDetails,status&playlistId=${uploadsPlaylistId}&maxResults=50&key=${apiKey}`,
      { headers: authHeader }
    );
    ...
    if (!videoIds) {
      return Response.json({ channelId: channelId_out, channelName, channelThumb, subscriberCount, videoCount, videos: [], oauthActive: !!accessToken });
    }

    // ── Video stats + privacy status ─────────────────────
